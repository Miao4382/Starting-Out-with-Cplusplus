...
int main()
{
	...
	//print out report
		//table head
	std::cout << " Student Name        Average Score       Grade\n"
		<< "--------------------------------------------------\n";

		//table content, use a loop to finish
	for (int i = 0; i < NUM_STUDENT; i++)
	{
		std::cout << std::setw(13) << name[i] << std::setw(19) << average[i] << std::setw(11) << grade[i] << std::endl;
	}
...
void average_score(double score[NUM_STUDENT][NUM_COURSE], double average[NUM_STUDENT])
{
	for (int counter_student = 0; counter_student < NUM_STUDENT; counter_student++)
	{
		...
		average[counter_student] = total / NUM_COURSE;
	}
}
