...
// Constants
const int MAX_COINS_CHANGE = 10000; // Max number of coins to give in change
  ...
  int numCoinsUsed)
{
  ...
  else if (amount == 0) // If solution is found
  {
    ...
    // print out solution
    std::cout << "Combination " << numSolutions << ": ";
    for (count = 0; count < numCoinsUsed; count++)
      std::cout << coinsUsed[count] << " ";
    std::cout << "\n";
  ...
  // Find the combinations not using the coin.
