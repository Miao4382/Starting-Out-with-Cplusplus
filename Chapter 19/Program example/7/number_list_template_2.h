...
private:
  ...
  T maxNode(const ListNode<T>* ptr) const; // recursively find out the maximum node
...
public:
  ...
  T MaxNode() const; // return the maximum value in all nodes, will call private function maxNode() and pass head to it. This is just an interface, which is used to pass head to the actual recursive function
...
template<class T>
T NumberList<T>::maxNode(const ListNode<T>* ptr) const
{
  // base case 
  if (ptr->next == nullptr)
    return ptr->value;
  
  // recursively calculate the maximum node 
  if (ptr->value >= maxNode(ptr->next))
    return ptr->value;
  else 
    return maxNode(ptr->next);
}
