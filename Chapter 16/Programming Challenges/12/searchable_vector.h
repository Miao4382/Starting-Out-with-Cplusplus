...
public:
  ...
  SearchableVector();
  // constructor with an integer and a T type argument 
  SearchableVector(int s, char value);
  // copy constructor 
  SearchableVector(const SearchableVector& obj);
  
  /* Function */
  int Find(const char keyword) const;
  
};

/* Constructor */
// default constructor
SearchableVector<char*>::SearchableVector() : SimpleVector<char*>()
{ }

// constructor with an integer and a T type argument 
SearchableVector<char*>::SearchableVector(int s, char value) : SimpleVector<char*>(s, value)
{ }

// copy constructor 
SearchableVector<char*>::SearchableVector(const SearchableVector& obj) : SimpleVector<char*>(obj)
{
  for (int i = 0; i < this->size(); i++)
    this->operator[](i) = obj[i];
...
/* Function */
int SearchableVector<char*>::Find(const char keyword) const
{
  if (this->operator[](0) >= this->operator[](size_ - 1)) // descending sort 
  {
    int lower = 0;
